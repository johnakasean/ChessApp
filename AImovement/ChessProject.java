   ...
   static int selected;
    ...
    public ChessProject(){
        ...
        for(int i=8;i < 16; i++){
            ...
            panels.add(pieces);//allowing pawn to show
        }
...
private Stack getWhitePawnSquares(int x, int y, String piece){
  ...
  Move validM, validM2, validM3, validM4; //pawns 4 moves
  for(int i = 1; i <=2; i++) {
      int tmpy = y + i;//temporary move for 1 or 2 on the y
      int tmpx = x;//temporary move of 0
      if(!(tmpy > 7)) {
          if(y == 1) {
      ...
      for (int i = 1; i < 2; i++){
          int tmpy = y + i;//temporary move for 1 on the y
          int tmpx = x;
          if (!(tmpy > 7)) {
              Square tmp = new Square(tmpx, tmpy, piece);
      ...
      for(int i = 1; i < 2; i++) {
          int tmpy = y + i;
          int tmpx = x + i;//temporary move for 1 on the x
...
*/
  ...
  System.exit(0);
    ...
    */
      ...
      // Starting the next best move method
      if(selected == 1 ){
       System.out.println("=============================================================");
       Border redBorder = BorderFactory.createLineBorder(Color.RED, 3);
    ...
    if(!validMove){
      ...
      if(progression){
        ...
        if ( c instanceof JLabel ){
          ...
          System.out.print("Success method should work (else)");
          Container parent = ( Container ) c;
          pieces = new JLabel( new ImageIcon( "WhiteQueen.png" ) );
          parent = (JPanel)chessBoard.getComponent(location);
          parent.add(pieces);
